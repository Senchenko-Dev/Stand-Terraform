...
resource "local_file" "elk-inventory" {
  depends_on = [null_resource.copy_group_vars]

  filename = "ansible/inventory/elk_${var.inventory_group_name}.ini"
  content = templatefile("tf_templates/elk/elk-inventory.tpl", // todo optimize
  {
    ans_hosts    = vcd_vm.elk,
  ...
  provisioner "ansible" {
    plays {
      ...
      inventory_file = local_file.elk-inventory.filename
    }
...
module "config_awx_ansible" {
  ...
  inventory_path = local_file.elk-inventory.filename
  inventory_group_name = var.inventory_group_name
  spo_role_name = var.spo_role_name
